...
<div id="couponContent" class="disf fw-w f-jcC">
...
<div id="gettedCouponContent" class="disf fw-w f-jcC">
...
<script>
    ...
    //讀出資料  Read
    fetch("./API/Coupon/Member_CouponGetRender_api.php").then(r => r.json()).then(res => {
        // console.log(res);
        if (res.length != 0) {
        ...
        res.coupons.forEach(value => {
            ...
            couponTicket.classList.add("pad10", "borderR1", "aic", "txtACenter", iro);
            ...
            if (!gettedOrNot) {
                ...
                getButton.addEventListener("click", () => {
                    memberPoint = Number(memberPoint);
                    need_point = Number(need_point);
                    if (memberPoint < need_point) {
                        console.log(memberPoint)
                        console.log(need_point)
                        alert("紅利點數不足，無法兌換!");
                        ...
                        fetch("./API/Coupon/Member_CouponGet_api.php", {
                                method: "POST",
                                body: FD
                            })
            ...
            //放入大框的位置
            if (gettedOrNot) {
                gettedCouponDocFrag.appendChild(framePad);
            } else {
                couponDocFrag.appendChild(framePad);
            }
        })

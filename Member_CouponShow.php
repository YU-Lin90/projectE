...
<div>
    <h2 class="txtACenter fs30 mb20">可使用的優惠券</h2>
...
<div>
    <h2 class="txtACenter fs30 mb20">
        已使用優惠券，只顯示近30天的資料
    </h2>
    <div id="usedCouponContent" class="disf fw-w f-jcC mb20">
...
<div>
    ...
    <div id="timeoutCouponContent" class="disf fw-w f-jcC mb20">
...
<script>
    ...
    //讀出資料  Read
    fetch("./API/Coupon/Member_CouponShowRender_api.php").then(r => r.json()).then(res => {
        // console.log(res);
        if (res.length != 0) {
        ...
        res.reverse().forEach(value => {
            ...
            if (continueOrNot == 1) {
                ...
                if (is_used == 0 && today - limitTime <= 0) {
                    // console.log("正常");
                    docFragCoupon.appendChild(framePad);
                }
                //B   已使用、    使用期限不管        使用時間三十日內
                else if (is_used == 1 && today - usedTime < day30ToSeconds) {
                    // console.log("已使用並在30日內");
                    docFragUsedCoupon.appendChild(framePad);
                }
                //C. 未使用  、 使用期限已過      無使用期間
                else if (is_used == 0 && today - limitTime > 0 && today - limitTime < day30ToSeconds) {
                    // console.log("已過期並在30日內");
                    docFragTimeoutCoupon.appendChild(framePad);
                }
